...
RUN addgroup -g 1000 admin && adduser -D -H -G rio -s /bin/false -u 1000 admin

# create a samba user matching our user from above with a very simple password ("admin")
RUN echo -e "admin\nadmin" | smbpasswd -a -s -c /config/smb.conf admin
